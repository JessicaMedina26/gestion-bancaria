...
class CreateCiudadView(generics.CreateAPIView):
    ...
    def post(self, request):
        ...
        if Ciudad.objects.filter(**request.data).exists():
            return Response({"message": "Ya existe la ciudad"}, status=status.HTTP_400_BAD_REQUEST)
...
class UpdateCiudadView(generics.UpdateAPIView):
    ...
    def put(self, request, pk):
        try:
            ...
            return Response({"message": "No se pudo identificar la Ciudad"}, status=status.HTTP_400_BAD_REQUEST)
